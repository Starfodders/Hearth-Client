...
const UnitSlide = ({ slide, unitID, currentSaved }) => {
  ...
  //save functionality. Toggles the save state and reacts accordingly
  // function handleSave() {
  ...
  if (type === "technique") {
    return (
      <TechniqueCard slide = {slide} format = {formatType} saveState = {isSaved} saveFunc = {setIsSaved} unitID = {unitID}/>
    );
  }
  // if (type === "list") {
  //   return (
  ...
  // }
  return (
    <TextCard slide = {slide} format = {formatType} saveState = {isSaved} saveFunc = {setIsSaved}/>
  );
};
